...
export default {
  ...
  validateResult(result) {
    ...
    // 守恒校验：得分不应高于「击虫*10 + 时间/2」过多（留取整余量）
    const expected = bugsKilled * 10 + Math.floor(timeSurvived / 2)
    if (score > expected + 50) return false
    if (timeSurvived > 86400) return false // 单局不超过 24h
    return true
  },

  // 由结果算官方排序分（前端仅显示，最终以服务端为准；与前端 computeScore 同公式）
  score(result) {
    const kill = (result.bugsKilled | 0) * 10
    const time = Math.floor((result.timeSurvived || 0) / 2)
    return kill + time
  },
